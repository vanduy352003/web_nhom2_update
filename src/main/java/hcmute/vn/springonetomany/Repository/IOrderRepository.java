...
public interface IOrderRepository extends JpaRepository<Order, Integer> {
	...
	// Thống kê doanh thu
	@Query("SELECT new hcmute.vn.springonetomany.Model.ProfitReportByDay(l.product.name, sum(l.price*l.quantity)) FROM Order o, OrderLines l WHERE o.create_at = ?1 AND l.order.id=o.id GROUP BY l.product.name, l.product.id")
	List<ProfitReportByDay> getProfitByDate(LocalDate date);

	@Query("SELECT sum(l.price*l.quantity) FROM Order o, OrderLines l WHERE o.create_at = ?1 AND l.order.id=o.id")
	Optional<Integer> findSumProfitByDate(LocalDate date);

	@Query("SELECT new hcmute.vn.springonetomany.Model.ProfitReportByMonth(o.create_at, sum(l.price*l.quantity)) FROM Order o, OrderLines l WHERE YEAR(o.create_at) = ?1 AND MONTH(o.create_at) = ?2 AND l.order.id=o.id GROUP BY o.create_at")
	List<ProfitReportByMonth> getProfitByMonth(int year, int month);

	@Query("SELECT sum(l.price*l.quantity) FROM Order o, OrderLines l WHERE YEAR(o.create_at) = ?1 AND MONTH(o.create_at) = ?2 AND l.order.id=o.id")
	Optional<Integer> findSumProfitByMonth(int year, int month);

	@Query("SELECT new hcmute.vn.springonetomany.Model.ProfitReportByYear(MONTH(o.create_at), sum(l.price*l.quantity)) FROM Order o, OrderLines l WHERE YEAR(o.create_at) = ?1 AND l.order.id=o.id GROUP BY MONTH(o.create_at)")
	List<ProfitReportByYear> getProfitByYear(int year);

	@Query("SELECT sum(l.price*l.quantity) FROM Order o, OrderLines l WHERE YEAR(o.create_at) = ?1 AND l.order.id=o.id")
